// Code generated by box-gantry 0.1.0 (spec ee7d55aedefe2fa0) for Box API 2024.0. DO NOT EDIT.
package dev.unofficialbox.model.schemas;

import module java.base;

public record AiItemAsk(String id, AiItemAskType type, Optional<String> content) {
    public java.util.Map<String, Object> toJson() {
        java.util.Map<String, Object> _m = new java.util.LinkedHashMap<>();
        _m.put("id", id());
        _m.put("type", (type() == null ? null : type().toJson()));
        content().ifPresent(_v -> _m.put("content", _v));
        return _m;
    }

    public static AiItemAsk fromJson(Object _json) {
        java.util.Map<String, Object> _m = dev.unofficialbox.core.Json.asObject(_json);
        return new AiItemAsk(
            dev.unofficialbox.core.Json.asString(_m.get("id")),
            (_m.get("type") == null ? null : AiItemAskType.fromJson(_m.get("type"))),
            (!_m.containsKey("content") || _m.get("content") == null) ? java.util.Optional.<String>empty() : java.util.Optional.of(dev.unofficialbox.core.Json.asString(_m.get("content")))
        );
    }

    /** A fluent builder; unset optional fields default to empty. */
    public static Builder builder() {
        return new Builder();
    }

    public static final class Builder {
        private String id;
        private AiItemAskType type;
        private Optional<String> content = java.util.Optional.empty();

        public Builder id(String id) {
            this.id = id;
            return this;
        }
        public Builder type(AiItemAskType type) {
            this.type = type;
            return this;
        }
        public Builder content(String content) {
            this.content = java.util.Optional.ofNullable(content);
            return this;
        }

        public AiItemAsk build() {
            return new AiItemAsk(
                id,
                type,
                content
            );
        }
    }
}
